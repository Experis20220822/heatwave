...
class LoginControllerSpec extends PlaySpec with GuiceOneAppPerTest with Results{
  ...
  "Login page GET" should {
    "render the service index" in {
      val controller = app.injector.instanceOf(classOf[LoginController])
      val result: Future[Result] = controller.index().apply(FakeRequest())
      contentAsString(result) must include("Use the same details you used to register for the service.")
    }
  }
}
